...
def addIt():
    if request.is_json:
        ...
        account = data.get('account')           #"account" cuz the keyword in json
    else:
        account = "OH"
...
def updateIt(id):
    if request.is_json:
        ...
        id = 1
        account = "OH"
    queryString = f"UPDATE Accounts SET Account = {account} WHERE Id = {id}"
    print(queryString)
    try:
        with dbc.cursor() as cursor:
            cursor.execute(queryString)
